...
| TPU grippers (v0.24) | 2× soft **TPU 95A** edge-grip + **teeth** + capture lip (`gripper_left/right`), print with the keymats | 2 | The phone's mechanical retention: compliant grip on the cased short edge + a deep lip that captures the screen edge. **v0.24d:** 13 half-round **teeth** (3.4 mm pitch, 0.8 mm proud, ribs running along the phone's thickness) bite the cased edge so the phone can't creep or rotate in y — friction alone wasn't holding it square. The lip gains a lead-in chamfer (snap the phone past it rather than threading it in square). Lip depth + clamp force are the **face-down-capture** tune (usable screen-down). |
| Extension springs (v0.24) | 2× stainless extension spring, ~5–8 N, ≥40 mm working extension | 2 | The clamp force, enclosed in the tray cavity, spanning grip-to-grip. **v0.24d:** they live in the two **OUTBOARD y lanes** (13.5 / 83.5) — never over a cable — and a wider stance also resists jaw racking. Force is the feel — coupon-tune. ⚠ The modelled anchor at x=22 gives a >5:1 working ratio across the 130–170 mm travel; move the anchor toward the tray's right wall (or pick a long, fine-wire spring) before buying. |
...
| MagSafe ring (v0.24, back) | Ø56 **N52** ring, 2.0 mm, in the tray-centre recess (0.2 proud) | 1 | **SECONDARY** back-hold + snap only — the clamp + lips retain the phone. Strong magnet is on OUR side (the phone/case magnet is the weak one). Opt-in: Android needs a magnetic case/ring; not load-bearing, so alignment drift across phones is a non-issue. |
...
| Shells | 7 printed parts: 6 in PETG — `back_left` (moving jaw), `back_right` (ground), `grip_lid_left`, `grip_lid_right`, the v0.24 `bridge` (fixed telescoping tray), + v0.21 `nub_spring` (Ø14.8 flexure, magnet press-pocket, v0.22 **4.4 mm-square TrackPoint cap platform** — arm thickness is the print-tune coupon) — and the v0.22 `nub_cap` (classic ThinkPad soft-dome replica, **RED TPU 95A**, prints with the keymats). **Or skip printing the cap: any genuine classic full-size TrackPoint cap (soft dome / soft rim / classic dome, ~4.5 mm square socket) fits the platform directly.** | 1 set | STLs tracked in `hardware/cad/models/` (regenerate: `deck3d.py --all --sync-models`). Every part fits an Ender 3 V2 (220 × 220) flat. **v0.24: `center_panel` retired → `bridge` tray; `gripper_left/right` (TPU) added.** Fit-checked 0 collisions across the 130–170 mm clamp travel (min/nominal/max). |
