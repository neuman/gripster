...
| Sync pinion + axle (v0.24) | printed spur gear (module 1.25, 14T, Ø3 bore) — nylon/POM or fine-layer PLA — on a **Ø3 dowel/screw axle** in the centre stage | 1 + 1 | Meshes the two grip racks (racks are printed into `back_left`/`back_right`) to enforce the 2:1 geared telescope. Model uses approximate straight-flank teeth; cut a true **involute** profile for production. |
| Extension springs (v0.24) | 2× stainless extension spring, ~5–8 N, ≥40 mm working extension | 2 | The clamp force. They run enclosed in the brace cavity, flanking the gear/rack lane, spanning grip-to-grip. Force is the feel — coupon-tune before committing. |
| Battery power cable (v0.24) | 2-wire, ~2 mm OD, ≥240 mm, JST-PH to the 403040 pigtail | 1 | Carries the left-grip battery to J3 on the right board — runs ENCLOSED through the telescoping shroud beside the FFC, with its own rolling service loop for the variable span. (Previously unmodeled.) |
